...
const UserControl = () => {
  ...
  useEffect(() => {
    setUsers(
      allUsers.data?.allUsers.filter((user) => user.rank.disabled === false)
    );
  }, [allUsers]);

  useEffect(() => {
    setUsersDisabled(
      allUsers.data?.allUsers.filter((user) => user.rank.disabled === true)
    );
  }, [allUsers]);

  useEffect(() => {
    setFilteredUsers(
      allUsers.data?.allUsers.filter((user) => user.email === userFilter)
    );
  }, [allUsers, userFilter]);
