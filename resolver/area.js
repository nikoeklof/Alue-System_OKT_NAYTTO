...
module.exports = {
  Query: {
    ...
    allAreas: async (root, args) => {
      ...
      if ("cityName" in args) newArgs["info.cityName"] = args.cityName;

      if ("quarter" in args) newArgs["info.quarter"] = args.quarter;

      if ("address" in args) newArgs["info.address"] = { $regex: args.address };

      if ("isShared" in args) newArgs["shareState.isShared"] = args.isShared;
  ...
  Area: {
    ...
    shareState: async (root) => {
      ...
      for (let x = 0; x < root.shareState.shareRequests.length; x++) {
        ...
        if (user) requestsArray.push(user.email);
      }
  ...
  Mutation: {
    ...
    editArea: async (root, args, contextValue) => {
      ...
      if (!area) throw new UserInputError("Area not found");
    ...
    deleteArea: async (root, args, contextValue) => {
      ...
      return await Area.findByIdAndDelete(args.areaId).catch((error) => {
        throw new UserInputError(error.message, {
          invalidArgs: args,
        });
      });
    ...
    removeRequest: async (root, args, contextValue) => {
      ...
      area.shareState.shareRequests.splice(
        area.shareState.shareRequests.indexOf(user._id),
        1
      );
    ...
    removeRequestAsAdmin: async (root, args, contextValue) => {
      ...
      if (!area.shareState.shareRequests.includes(user._id))
        throw new UserInputError(
          "This has not been requested by: " + args.email
        );
